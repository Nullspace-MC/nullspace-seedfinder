...
#ifdef USE_PTHREAD
void *findOtherStructuresThread(void *arg) {
#else
DWORD WINAPI findOtherStructuresThread(LPVOID arg) {
#endif
...
int main(int argc, char *argv[]) {
    ...
    const char *base_list_path = NULL;
    ...
    int64_t scnt, *seeds;
    seeds = loadSavedSeeds(base_list_path, &scnt);
    if(seeds == NULL) {
